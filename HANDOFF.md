...
Session 2026-06-21 (autopilot, gate-only run): no queue tasks pending. Inspected the 3 newest recovery branches — the two newest (20260620, 20260621) hold only `.harness/autopilot.lock`, no usable work; the 20260615 branch's real change was already tracked in a prior HANDOFF. Ran gate-1-code-green.ps1; it now reported a NEW failure "working tree dirty" caused by the untracked `.harness/` wrapper-state dir (lock + logs), which the wrapper writes into the app tree each run. Fixed by adding `.harness/` to `.gitignore` (ungoverned surface) — this also stops interrupted runs from spawning lock-file-only recovery branches. Tree is now clean. Regenerated ops/telemetry/quality-report.md. Remaining gate failures, all needs-owner and unchanged from prior runs: (1) coverage 74.29% vs 80% floor — proposal at ops/proposals/gate-1-coverage-floor.md; (2) ruff not clean (errors outside postmule/, which is clean); (3) mypy exit non-zero; (4) bandit 0 High/Medium but gate fails on any output; (5) pip 26.0.1 CVEs — upgrade blocked on p1-fix-safe-pip; (6) eight open issues (#30, #87, #91, #93, #96, #97, #104, #107) lack `blocked:human`/`post-release` labels.
