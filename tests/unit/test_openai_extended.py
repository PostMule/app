"""Extended tests for postmule.providers.llm.openai (classify, _parse_response, _safe_float)."""
...
from postmule.providers.llm.openai import (
    OpenAIProvider,
    _parse_response,
    _safe_float,
)
...
class TestOpenAIProviderDryRun:
...
class TestGetClient:
    def test_get_client_raises_when_openai_not_installed(self):
        provider = OpenAIProvider(api_key="test-key")
...
class TestOpenAIClassifyWithMockedClient:
    def _make_response(self, payload, total_tokens=100):
    ...
    def test_calls_safety_agent_before_api(self):
        safety = MagicMock()
        provider = OpenAIProvider(api_key="key", safety_agent=safety)
    ...
    def test_no_usage_falls_back_to_estimate(self):
        provider = OpenAIProvider(api_key="key")
        mock_client = MagicMock()
    ...
    def test_api_error_raises_runtime_error(self):
        ...
        with pytest.raises(RuntimeError, match="OpenAI classification failed"):
            provider.classify("some text")
